...
Router.get('/account', authMiddleware, async (req, res) => {
  try {
    ...
    if (result) {
      ...
      res.status(400).send({
        get_error: 'Account details do not not exist.'
      });
    }
  } catch (error) {
    res.status(400).send({
      get_error: 'Error while getting account details.. Likely connection issue.'
    });
  }
});
...
Router.post('/account', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res.send({
      add_error: 'Error while adding a new account..Try again later.'
    });
  }
});
