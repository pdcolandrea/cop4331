...
Router.post('/signup', async (req, res) => {
  try {
    ...
    res.status(400).send({
      signup_error: 'Likely error connecting to database.'
    });
  }
});
...
Router.post('/signin', async (req, res) => {
  try {
    ...
    if (!user) {
      res.status(400).send({
        sigin_error: 'Email or password is incorrev'
      });
    }
...
Router.post('/logout', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(400).send({
      logout_error: 'Error while logging out..Try again later. You should not ever be here, basiccaly.'
    });
  }
});
